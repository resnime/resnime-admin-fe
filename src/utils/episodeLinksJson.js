...
export function parseEpisodeLinksJson(fileText) {
  ...
  parsed.forEach((item, index) => {
    ...
    if (episodeNumber === null) {
      errors.push(`Episode item ${index + 1}: episode_number must be a positive integer.`);
      return;
    }
    ...
    if (item.links.length === 0) {
      errors.push(`Episode ${episodeNumber}: links must contain at least one valid link.`);
    }
...
export function mergeEpisodeLinks({
  ...
  const currentByNumber = new Map(
    currentEpisodes
      ...
      .map((episode) => [Number(episode.episode_number), cloneEpisode(episode)]),
  );
  ...
  importedEpisodes.forEach((importedEpisode) => {
    ...
    const seenLinks = new Set(currentEpisode.links.map((link) => link.embed_url));
  ...
  const currentTotal =
    Number.isInteger(Number(currentEpisodeTotal)) && Number(currentEpisodeTotal) > 0
      ? Number(currentEpisodeTotal)
      : null;

  return {
    episodes,
    episode_total: currentTotal === null ? highestEpisodeNumber || null : Math.max(currentTotal, highestEpisodeNumber),
  };
}
...
export function calculateEpisodeImportSummary({
  ...
  const importedNumbers = importedEpisodes.map((episode) => episode.episode_number);
  ...
  return {
    ...
    duplicateEpisodesMerged,
    duplicateLinksRemoved,
    newEpisodesToCreate: importedNumbers.filter((number) => !currentNumbers.has(number)).length,
    existingEpisodesToUpdate: importedNumbers.filter((number) => currentNumbers.has(number)).length,
...
function normalizeLinks(links, episodeNumber, errors) {
  ...
  links.forEach((link, index) => {
    const embedUrl = typeof link?.embed_url === "string" ? link.embed_url.trim() : "";
...
function normalizeThumbnailUrl(value, episodeNumber, warnings) {
  ...
  warnings.push(`Episode ${episodeNumber}: thumbnail_url was ignored because it is not a valid HTTP or HTTPS URL.`);
  return null;
}
...
function cloneEpisode(episode) {
  return {
    ...
    links: Array.isArray(episode.links)
      ? episode.links
          .map((link) => ({ embed_url: typeof link?.embed_url === "string" ? link.embed_url.trim() : "" }))
